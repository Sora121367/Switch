...
const SidebarItem = ({ icon: Icon, label, path, isActive, onClick, subItems, activeItem, setActiveItem }) => (
      ...
      <ul className="ml-6 mt-2">
        {subItems.map((subItem, index) => (
          <li
            ...
            onClick={() => setActiveItem(subItem.label)} // Update the active state
          >
  ...
  <aside className="w-[300px] h-screen max-sm:hidden flex flex-col p-4 shadow-md rounded-tl-xl bg-white overflow-hidden">
    {items.map((item, index) => (
      <SidebarItem
        ...
        setActiveItem={setActiveItem} // Pass down setter
